...
export function detectObfuscationTechniques(input) {
    ...
    if (!input || typeof input !== 'string') {
        console.warn('[SECURITY] detectObfuscationTechniques received invalid input type:', typeof input);
        return { 
    ...
    try {
        ...
        console.error('[SECURITY] Error in detectObfuscationTechniques:', error);
        return { 
            hasObfuscation: false, 
            techniques: {},
            error: error.message 
...
export function analyzeInputStructure(input) {
    // Add robust type checking
    if (!input || typeof input !== 'string') {
        console.warn('[SECURITY] analyzeInputStructure received invalid input type:', typeof input);
        return { 
    ...
    try {
        ...
        // Check for payload sections - use safe string operation
        const hasPayload = input.indexOf('\n\n') !== -1 || 
                           input.indexOf('```') !== -1 || 
                           input.indexOf('---') !== -1;
        ...
        console.error('[SECURITY] Error in analyzeInputStructure:', error);
        return { 
            suspiciousStructure: false,
            structureScore: 0,
            factors: {},
...
export class ContextTracker {
    ...
    updateState(input, inputAnalysis) {
        try {
            ...
            // Safely access inputAnalysis properties with defaults
            const isJailbreakAttempt = inputAnalysis && inputAnalysis.isJailbreakAttempt || false;
            const score = inputAnalysis && inputAnalysis.score || 0;
            
            // Update based on current input analysis
            if (isJailbreakAttempt || score > 50) {
            ...
            console.error('[SECURITY] Error in ContextTracker.updateState:', error);
    ...
    analyzeSequentialPatterns() {
        try {
            ...
            console.error('[SECURITY] Error in analyzeSequentialPatterns:', error);
            return {
                hasPattern: false,
                error: error.message
            };
...
export function detectRoleplayManipulation(input) {
    // Add robust type checking
    if (!input || typeof input !== 'string') {
        console.warn('[SECURITY] detectRoleplayManipulation received invalid input type:', typeof input);
        return { 
    ...
    try {
        ...
        console.error('[SECURITY] Error in detectRoleplayManipulation:', error);
        return { 
            isManipulation: false,
            matchCount: 0,
            matches: [],
...
export function analyzeLanguageComplexity(input) {
    // Add robust type checking
    if (!input || typeof input !== 'string') {
        console.warn('[SECURITY] analyzeLanguageComplexity received invalid input type:', typeof input);
        return { 
            isComplex: false, 
            complexity: 0,
            details: {},
    ...
    try {
        ...
        console.error('[SECURITY] Error in analyzeLanguageComplexity:', error);
        return { 
            isComplex: false, 
            complexity: 0,
            details: {},
